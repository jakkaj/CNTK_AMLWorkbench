...
def prepare(cfg, use_arg_parser=True):
    ...
    cfg['BASE_MODEL_PATH'] = os.path.join(os.environ['AZUREML_NATIVE_SHARE_DIRECTORY'], cfg["MODEL"].BASE_MODEL_FILE)
...
def train_faster_rcnn(cfg):
    ...
    model_path=os.environ['AZUREML_NATIVE_SHARE_DIRECTORY'] + "animals.model"
    ...
    if os.path.exists(model_path) and cfg["CNTK"].MAKE_MODE:
        ...
        print("Stored eval model at %s" % save_path)
    return eval_model
